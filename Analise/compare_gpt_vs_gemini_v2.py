...
--input_glob "D:\FURG 2025\TCC\parte pratica\IA user\JSON_Conversas" \
--output_dir "./out" \
--gpt_out_per_m 1.60 \
...
python compare_gpt_vs_gemini_v2.py \
--input_glob "/path/conversa_*.json" \
--output_dir "./out" \
...
def save_boxplot(values_by_label: Dict[str, List[float]], title: str, ylabel: str, out_png: str) -> None:
    labels = list(values_by_label.keys())
    data = [values_by_label[k] for k in labels]
    plt.figure()
...
def save_hist(values: List[float], title: str, xlabel: str, out_png: str, bins: int = 20) -> None:
    plt.figure()
    plt.hist([v for v in values if v is not None and not (isinstance(v, float) and math.isnan(v))], bins=bins)
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel("Count")
    plt.tight_layout()
...
def save_scatter(x: List[float], y: List[float], title: str, xlabel: str, ylabel: str, out_png: str) -> None:
    xs, ys = [], []
    for a, b in zip(x, y):
...
def main() -> None:
    ...
    ap.add_argument("--input_glob", required=True, help="Glob for JSON logs, e.g., '/path/conversa_*.json'")
    ...
    save_boxplot(dt, "Decision turn (trimmed at decision)", "Turn index", os.path.join(args.output_dir, "decision_turn_box_trimmed.png"))

    # output_tokens_used
    tok = by_model("output_tokens_used")
    save_boxplot(tok, "Output tokens (USED segment only)", "Approx output tokens", os.path.join(args.output_dir, "output_tokens_box_trimmed.png"))
    save_hist(tok["gpt"] + tok["gemini"], "Output tokens distribution (USED)", "Approx output tokens", os.path.join(args.output_dir, "output_tokens_hist_trimmed.png"))

    # cost (output-only)
    if args.gpt_out_per_m is not None and args.gemini_out_per_m is not None:
        cost = by_model("cost_used_usd")
        save_boxplot(cost, "Estimated cost USD (output tokens, USED only)", "USD", os.path.join(args.output_dir, "cost_box_trimmed.png"))

    # repetitiveness
    rep = by_model("repetitiveness_used")
    save_boxplot(rep, "Repetitiveness (Jaccard, USED only)", "Jaccard similarity", os.path.join(args.output_dir, "repetitiveness_box_trimmed.png"))
    ...
    save_scatter(x_g, y_g, "GPT: decision turn vs tokens (USED)", "Decision turn", "Output tokens (USED)", os.path.join(args.output_dir, "gpt_decision_vs_tokens.png"))
    save_scatter(x_m, y_m, "Gemini: decision turn vs tokens (USED)", "Decision turn", "Output tokens (USED)", os.path.join(args.output_dir, "gemini_decision_vs_tokens.png"))
    ...
    if args.ratings_csv:
        ...
        for d in dims:
            ...
            save_boxplot({"gpt": gvals, "gemini": mvals},
                         f"Qualitative: {d}", d, os.path.join(args.output_dir, f"qual_{d}_box.png"))
